control c_mv(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    c_sketch_read()	mv_read_0;
    c_sketch_read()	mv_read_1;
    c_sketch_read()	mv_read_2;
    c_sketch_read()	mv_read_3;
    ...
    apply {
        ...
        mv_read_1.apply(hdr, meta, standard_metadata);

        reg_key_0_temp = meta.reg.sketch_temp;

        // Register Key 1

        mv_set_reg_2.apply(hdr, meta, standard_metadata);

        mv_read_2.apply(hdr, meta, standard_metadata);

        reg_key_1_temp = meta.reg.sketch_temp;
        ...
        if (meta.mv.key_temp == hdr.ipv4.src_addr ++ hdr.ipv4.dst_addr || meta.mv.count_temp == 0) {
            meta.reg.sketch_temp = meta.mv.count_temp + 1;
        } else {
            meta.reg.sketch_temp = meta.mv.count_temp - 1;
        }
