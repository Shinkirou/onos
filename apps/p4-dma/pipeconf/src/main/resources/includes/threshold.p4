control c_threshold(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
	...
	register<bit<32>>(262144) traffic_global_flow_register;
	// Stores the packet count for the current stage (for each flow).
	register<bit<32>>(262144) traffic_flow_register;
	// Store the timestamp corresponding to the beginning of the current stage (for each flow).
	register<bit<48>>(262144) time_flow_register;

	bit<32> cm_5t_sketch_final;
	bit<32> cm_ip_sketch_final;
	bit<32> bm_src_sketch_final;
	bit<32> bm_dst_sketch_final;
	bit<32> ams_sketch_final;
	...
	action send_to_cpu_threshold() {
		...
		hdr.packet_in.cm_5t         = cm_5t_sketch_final;		
		hdr.packet_in.cm_ip         = cm_ip_sketch_final;
		hdr.packet_in.bm_src        = bm_src_sketch_final;
		hdr.packet_in.bm_dst        = bm_dst_sketch_final;
		hdr.packet_in.ams           = ams_sketch_final;
		...
		@atomic {
			...
			if ((standard_metadata.ingress_global_timestamp - meta.threshold.flow_time) > (bit<48>)5000000) {
				...
				if ((meta.threshold.flow_traffic * 10) > (meta.threshold.global_traffic - meta.threshold.flow_global_traffic)) {
					...
					cm_5t_sketch_final = meta.cm_5t.sketch_final;
					cm_ip_sketch_final = meta.cm_ip.sketch_final;
					bm_src_sketch_final = meta.bm_src.sketch_final;
					bm_dst_sketch_final = meta.bm_dst.sketch_final;
					ams_sketch_final = meta.ams.sketch_final;

					if (meta.epoch.current_epoch == 1) {

						cm_5t_sketch_final[31:31] = (bit<1>)0;
						cm_ip_sketch_final[31:31] = (bit<1>)0;
						bm_src_sketch_final[31:31] = (bit<1>)0;
						bm_dst_sketch_final[31:31] = (bit<1>)0;
						ams_sketch_final[31:31] = (bit<1>)0;
					}
