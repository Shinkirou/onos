...
parser c_parser(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_tcp {
        ...
        meta.metadata_packet.l4_src_port = hdr.tcp.src_port;
        meta.metadata_packet.l4_dst_port = hdr.tcp.dst_port;
        transition accept;
    }

    state parse_udp {
        packet.extract(hdr.udp);
        meta.metadata_packet.l4_src_port = hdr.udp.src_port;
        meta.metadata_packet.l4_dst_port = hdr.udp.dst_port;
        transition accept;
    }
}
