control c_bm_src(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
	...
	register<bit<32>>(REG_SKETCH_SIZE)  reg_bm_src_0;
	// Bitmap register for the source address.
	register<bit<32>>(REG_SKETCH_SIZE)  reg_bm_src_1;
	...
	action bm_check_pair() {
		...
		reg_bm_src_0.read(meta.bm_src.sketch_0, (bit<32>)meta.bm_src.hash_0);
	}

	action bm_new_pair() {
		
		reg_bm_src_1.read(meta.bm_src.sketch_1, (bit<32>)meta.bm_src.hash_1);

		meta.bm_src.sketch_1 = meta.bm_src.sketch_1 + 1;

		reg_bm_src_0.write((bit<32>)meta.bm_src.hash_0, 1);
		reg_bm_src_1.write((bit<32>)meta.bm_src.hash_1, meta.bm_src.sketch_1);
	}
